 ...
 * @return
 */
static inline evm_val_t evm_mk_foreign_string(intptr_t v) {
    return INNER_TYPE_STRING_F | (v & EVM_ADDR_MASK);
}
/**
 * @brief 创建堆字符串值
 * @param 堆字符串对象
...
static inline void evm_set_foreign_string(evm_val_t *v, intptr_t s) {
    *((uint64_t *)v) = INNER_TYPE_STRING_F | (s & EVM_ADDR_MASK);
}
/**
 * @brief 设置值为堆字符串
 * @param v
...
static inline void evm_set_class(evm_val_t *v, intptr_t s) {
    *((uint64_t *)v) = INNER_TYPE_CLASS | ((intptr_t)s & EVM_ADDR_MASK);
}
/**
 * @brief 设置值为字节数组
 * @param v
